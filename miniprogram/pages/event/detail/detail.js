...
Page({
  data: { event: null, loading: true, error: '', isReserved: false, reserving: false, completing: false, showCompleteForm: false, did: '', learned: '' },
  onLoad(options) {
    if (options.id) { this.eventId = options.id; this.loadEvent(); this.checkReservation() }
  },
  async loadEvent() {
    ...
    try {
      ...
      } else { this.setData({ error: '事件不存在', loading: false }) }
    } catch (err) { this.setData({ error: err.message || '加载失败', loading: false }) }
  },
  async checkReservation() {
    if (!app.globalData.token) return
    try {
      const reservations = await get('/api/events/reservations')
      this.setData({ isReserved: (reservations || []).some(r => r.event?.id == this.eventId) })
    } catch (e) {}
  },
  async onReserve() {
    ...
    try {
      await post(`/api/events/${this.eventId}/reserve`)
      this.setData({ isReserved: true, reserving: false }); showSuccess('预约成功')
    } catch (err) { this.setData({ reserving: false }); showError(err.message || '预约失败') }
  },
  ...
  async onConfirmComplete() {
    ...
    try {
      await post(`/api/events/${this.eventId}/complete`, { did: this.data.did, learned: this.data.learned })
      this.setData({ completing: false, showCompleteForm: false }); showSuccess('完成记录已保存')
      setTimeout(() => wx.navigateBack(), 1500)
    } catch (err) { this.setData({ completing: false }); showError(err.message || '记录失败') }
  },
  onShareAppMessage() {
    return { title: this.data.event?.title || 'Get Yourself', path: `/pages/event/detail/detail?id=${this.eventId}` }
  }
})
